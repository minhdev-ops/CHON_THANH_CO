...
@section('content')
    <div class="max-w-3xl">
        <form method="POST" action="{{ $category ? route('admin.news-categories.update', $category) : route('admin.news-categories.store') }}"
            x-data="{ lang: 'vi' }">
            @csrf
            @if ($category)
                @method('PUT')
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.news-categories.index') }}" class="text-gray-600 hover:underline">Hủy</a>
            </div>
        </form>
    </div>
@endsection
