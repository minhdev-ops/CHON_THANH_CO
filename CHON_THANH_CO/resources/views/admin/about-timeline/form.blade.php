...
@section('content')
    <div class="max-w-3xl">
        <form method="POST" action="{{ $item ? route('admin.about-timeline.update', $item) : route('admin.about-timeline.store') }}"
            x-data="{ lang: 'vi' }">
            @csrf
            @if ($item)
                @method('PUT')
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6 mb-6">
                ...
                <x-admin.partials.translation-section
                    ...
                    label="Tiếng Việt"
                    :fields="[
                        ['name' => 'year', 'label' => 'Mốc thời gian', 'required' => true, 'max' => 100, 'hint' => 'VD: 2005 - Thành lập'],
                ...
                <x-admin.partials.translation-section
                    ...
                    label="English"
                    :fields="[
                        ['name' => 'year', 'label' => 'Year milestone', 'max' => 100, 'hint' => 'e.g. 2005 - Founding'],
            ...
            <div class="flex items-center gap-3">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2.5 rounded">
                    {{ $item ? 'Lưu thay đổi' : 'Tạo mốc' }}
                </button>
                <a href="{{ route('admin.about-timeline.index') }}" class="text-gray-600 hover:underline">Hủy</a>
            </div>
        </form>
    </div>
@endsection
